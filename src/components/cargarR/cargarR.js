...
function CargarRiver(){
    const [formData,setFormData]=useState({});
    const [metodo,setMetodo]=useState("");
    const [isOk,setIsOk]=useState(false);

const cargarRiv=async(e)=>{
    e.preventDefault();
    ...
    await fetch(`https://api-jugadores-u6de.onrender.com/cargarRiver/${idJugador}`, objetoFetch)
...
return(
    ...
    {isOk === false?
        <form className="riv" onSubmit={(event)=>cargarRiv(event)}>
    ...
    <input className="btn btn-succes d-block" type="submit" value={"cargar"} />
    
    </form>:
    
    <p className="p-2 text-center cargado ">Jugador Cargado!!</p>
    }
